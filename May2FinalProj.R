...
#AntiBlackGraphs
datahatecrime$Anti.Black <- as.numeric(datahatecrime$Anti.Black)

anti_black <- select(datahatecrime, "Crime.Type", "Trumpelec", "Anti.Black") %>% distinct() 
anti_black <- anti_black[-44, ]

ggplot(anti_black, aes(Crime.Type, Anti.Black)) +
  geom_boxplot() +
  facet_wrap(~Trumpelec)

#AntiAsianGraphs
datahatecrime$Anti.Asian <- as.numeric(datahatecrime$Anti.Asian)

anti_asian <- select(datahatecrime, "Crime.Type", "Trumpelec", "Anti.Asian") %>% distinct() 
anti_asian <- anti_asian[-44, ]

ggplot(anti_asian, aes(Crime.Type, Anti.Asian)) +
  geom_boxplot() +
  facet_wrap(~Trumpelec)

#AntiHispanicGraphs
datahatecrime$Anti.Hispanic <- as.numeric(datahatecrime$Anti.Hispanic)

anti_hispanic <- select(datahatecrime, "Crime.Type", "Trumpelec", "Anti.Hispanic") %>% distinct() 
anti_hispanic <- anti_hispanic[-20, ]

ggplot(anti_hispanic, aes(Crime.Type, Anti.Hispanic)) +
  geom_boxplot() +
  facet_wrap(~Trumpelec)
